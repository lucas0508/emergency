...
public class ClusterOverlay implements AMap.OnCameraChangeListener,
    ...
    public ClusterOverlay(AMap amap, int clusterSize, Context context) {
        this(amap, null, clusterSize, context, Constants.MAPROLE_DOCTOR);
    ...
    public ClusterOverlay(AMap amap, List<ClusterItem> clusterItems,
                          int clusterSize, Context context, int mapRole) {
...
//            if (mClusterRender != null && mClusterRender.getDrawAble(num,mMapRole) != null) {
//                textView.setBackgroundDrawable(mClusterRender.getDrawAble(num,mMapRole));
//            } else {
    ...
    private BitmapDescriptor getBitmapDes(int num, Cluster cluster) {
        ...
        if (num > 1) {
            ...
            if (Constants.MAPROLE_DOCTOR == mMapRole) {
                ...
                textView.setBackgroundResource(R.mipmap.icon_positioning_doctor);


            } else if (Constants.MAPROLE_VOLUNTEER == mMapRole) {

                textView.setBackgroundResource(R.mipmap.icon_positioning_volunteers);
            } else if (Constants.MAPROLE_AED == mMapRole) {
                textView.setBackgroundResource(R.mipmap.icon_positioning_aid);
            }
//            textView.setGravity(Gravity.CENTER);
//            textView.setTextColor(Color.BLACK);
//            textView.setTextSize(TypedValue.COMPLEX_UNIT_SP, 15);
...
//            }
            /*if (mClusterRender != null && mClusterRender.getDrawAble(num,mMapRole) != null) {
                view.setBackgroundDrawable(mClusterRender.getDrawAble(num,mMapRole));
            } else {
                view.setBackgroundResource(R.mipmap.defaultcluster);
            }*/
            bitmapDescriptor = BitmapDescriptorFactory.fromView(textView);
            if (bitmapDescriptor == null) {
                mLruCache.put(num, bitmapDescriptor);
